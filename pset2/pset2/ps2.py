...
class RectangularRoom(object):
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        if 0 > pos.getX() or pos.getX() >= self.width:
            return False
        if 0 > pos.getY() or pos.getY() >= self.height:
            return False
        return True
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for h in range(num_trials):
        ...
        for i in range(num_robots):
            robots.append(robot_type(room = room, speed = speed))
    ...
    return sum(tickList) / len(tickList)
...
#     plot.
title = 'time for 1-10 robots to clean 80% of a room'
...
#     plot.
title = 'time for 2 robots to clean 80% of variously shaped rooms'
